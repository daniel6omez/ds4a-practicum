...
class TwStreamListener(tweepy.StreamListener):
    ...
    def on_status(self, status):
        ...
        text = self.deEmojify(status.text)  
          # Pre-processing the text  
        
        analysis =  TextBlob(text).translate(to='en')
        ...
        try:
            ...
            Check if this table exits. If not, then create a new one.
            '''
            if polarity < 0:  polarity = np.floor(polarity)
            if polarity > 0: polarity = np.ceil(polarity)
            ...
            val = (id_str, created_at, text, polarity, round(subjectivity,0), user_created_at, user_location, \
    ...
    def run(self):
        ...
        self.myStream.filter(languages=["es"], track = settings.TRACK_WORDS,is_async=True)
   ##     self.myStream.filter(languages=["es"], follow=["73007294","40031040","497511506","881625072"] ,is_async=True)
